...
def test_configure_dataset(rna):
    ...
    with pytest.raises(ValueError):
        scglue.models.configure_dataset(
            rna, "NB", use_highly_variable=False, use_layer="xxx"
        )
    with pytest.raises(ValueError):
        scglue.models.configure_dataset(
            rna, "NB", use_highly_variable=False, use_rep="xxx"
        )
    with pytest.raises(ValueError):
        scglue.models.configure_dataset(
            rna, "NB", use_highly_variable=False, use_dsc_weight="xxx"
        )
    with pytest.raises(ValueError):
        scglue.models.configure_dataset(
            rna, "NB", use_highly_variable=False, use_cell_type="xxx"
        )
...
def test_fit_SCGLUE(rna_pp, atac_pp, guidance, use_df):
    if use_df:
        rna_pp.obsm["X_pca"] = pd.DataFrame(
            rna_pp.obsm["X_pca"], index=rna_pp.obs_names
        )
    scglue.models.configure_dataset(
        rna_pp,
        "NB",
...
def test_fit_SCGLUE_with_align_support(rna_pp, atac_pp, guidance):
    scglue.models.configure_dataset(
        rna_pp,
        "NB",
        use_highly_variable=True,
    ...
    scglue.models.fit_SCGLUE(
        ...
        compile_kws={"lr": 1e-5},
        fit_kws={"max_epochs": 5},
        align_support_kws={"n_neighbors": 2},
    )  # NOTE: Smoke test
    for adata in (rna_pp, atac_pp):
        assert "unsupported_score" in adata.obs
        assert "unsupported_align_weight" in adata.obs
        assert "combined_dsc_weight" in adata.obs
        assert (
            adata.uns[scglue.config.ANNDATA_KEY]["use_dsc_weight"]
            == "combined_dsc_weight"
        )
