...
def extract_rank_genes_groups(
    ...
    df = (
        ...
        .sort_values(["group", sort_by], ascending=[True, ascending])
        .query(filter_by)
    )
...
def bedmap2anndata(bedmap: os.PathLike, var_col: int = 3, obs_col: int = 6) -> AnnData:
    r"""
    ...
    *adatas
    ...
    if common:
        common_metacells = list(
            set.intersection(*(set(adata.obs_names) for adata in adatas))
        )
...
def metacell_corr(
    ...
    **kwargs
    ...
    return _metacell_corr(*adatas, skeleton=skeleton, method=method, prep_fns=prep_fns)
...
def metacell_regr(
    ...
    **kwargs
    ...
    metacell_regr.logger.info(
        "Computing regression on %d common metacells...", adatas[0].shape[0]
    )
